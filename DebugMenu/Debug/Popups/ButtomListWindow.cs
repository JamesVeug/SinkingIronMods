...
namespace DebugMenu.Scripts.Popups
{
	...
	public class ButtonListPopup : BaseWindow
	{
		...
		public override void OnGUI()
		{
			...
			Rect scrollableAreaSize = new Rect(new Vector2(0, 0),
				new Vector2(columns * ColumnWidth + (columns - 1) * 10, rows * RowHeight));
			...
			Label("Filter", new Vector2(0, RowHeight / 2));
			filterText = TextField(filterText, new Vector2(0, RowHeight / 2));
		...
		public static bool OnGUI(DrawableGUI gui, string buttonText, string headerText,
			Func<Tuple<List<string>, List<string>>> GetDataCallback, Action<int, string, string> OnChoseButtonCallback,
